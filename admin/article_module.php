...
function make_post_table($rs,$tplcode=''){
	...
	if($rs[form_type]=='text')	
	{
		...
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}：</td> <td  class='tl'> <input type='text' name='post_db[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='{$rs[field_inputleng]}' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units] {$rs[form_title]}</td></tr>";	
	}
	elseif($rs[form_type]=='time')	
	{	
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}： </td> <td  class='tl'> <input  onclick=\"setday(this,1)\" type='text' name='post_db[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='20' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units] {$rs[form_title]}</td></tr>";	
	}
	elseif($rs[form_type]=='upfile')	
	{	
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}：<br>{$rs[form_title]}</td> <td  class='tl'> <input type='text' name='post_db[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='50' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units]<br><iframe frameborder=0 height=23 scrolling=no src='upfile.php?fn=upfile&dir=\$_pre\$fid&label=atc_{$rs[field_name]}&ISone=1' width=310></iframe> </td></tr>";	
	}
	elseif($rs[form_type]=='upplay')	
	{	
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}：<br>{$rs[form_title]}</td> <td  class='tl'>
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}：<br>{$rs[form_title]}  <a href='javascript:showinput_{$rs[field_name]}()'>添加远程视频</a></td> <td  class='tl'>
		<!--\r\nEOT;\r\n
...
print <<<EOT\r\n-->注意:服务器限制单个视频文件大小不能超过 <font color=red>{\$max_upload}</font>
	...
	var so = new SWFObject(\"\$webdb[www_url]/images/default/uploadmore.swf\", \"sap_load\", \"450\", \"30\", \"9\", \"#ffffff\");
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}：<br>{$rs[form_title]} <a href='javascript:showinput_{$rs[field_name]}()'>添加远程文件</a></td> <td>
<!--\r\nEOT;\r\n
\$upfiletype=str_replace(' ',',',trim(\$groupdb[upfileType]?\$groupdb[upfileType]:\$webdb[upfileType]));
\$upfiletype=str_replace('.','',\$upfiletype);
\$max_upload=ini_get('upload_max_filesize')?ini_get('upload_max_filesize'):'0';
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}：<br>{$rs[form_title]}<a href='javascript:showinput_{$rs[field_name]}()'>添加远程图片</a></td> <td  class='tl'>
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<tr><td  class='tr'>{$mustfill}{$rs[title]}： </td><td ><textarea name='post_db[{$rs[field_name]}]' id='atc_{$rs[field_name]}' cols='70' rows='8'>\$rsdb[{$rs[field_name]}]</textarea>$rs[form_units] {$rs[form_title]}</td></tr>";	
	}	
	elseif($rs[form_type]=='ieedit')	
	{	
		$show="<tr><td  class='tr'>{$mustfill}{$rs[title]}：<br>{$rs[form_title]}</td><td ><iframe id='eWebEditor1' src='ewebeditor/ewebeditor.php?id=atc_{$rs[field_name]}&style=standard&etype=1' frameborder='0' scrolling='no' width='630' height='200'></iframe>$rs[form_units]<input name='post_db[{$rs[field_name]}]' id='atc_{$rs[field_name]}' type='hidden' value='\$rsdb[{$rs[field_name]}]'></td></tr>";	
	}	
	...
	{	
		...
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}： </td><td > <select name='post_db[{$rs[field_name]}]' id='atc_{$rs[field_name]}'>$_show</select>$rs[form_units] {$rs[form_title]}</td> </tr>";	
	}	
	...
	{	
		...
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}： </td> <td  class='tl'>$_show$rs[form_units] {$rs[form_title]}</td></tr>";	
	}	
	...
	{	
		...
		$show="<tr> <td  class='tr'>{$mustfill}{$rs[title]}：<br>{$rs[form_title]}</td> <td  class='tl'>$_show$rs[form_units] {$rs[form_title]}</td></tr>";
	}
	return $show;
}
...
{	
	...
	{	
		...
		foreach( $detail AS $key=>$value){	
			...
			$v2 || $v2=$v1;	
			$_show.="
			<input type='checkbox' name='postdb[{$rs[field_name]}][]' value='$v1' {\$rsdb[{$rs[field_name]}]['{$v1}']}>$v2 ";	
		}	
		$show="$_show";			
	}
	else	
