...
if($action=="postnew")
{
	...
	**/
	...
	$fidDB[email] && send_mail($fidDB[email],"来自{$fidDB[name]}的邮件：$postdb[title]","$postdb[content]<br><hr><a href='$webdb[admin_url]/index.php?lfj=module_admin&dirname={$ModuleDB[$webdb[module_pre]][dirname]}&file=form_content&job=view&mid=$fidDB[id]&id=$id'>要查看更多详情，请点击$webdb[admin_url]/index.php?lfj=module_admin&dirname={$ModuleDB[$webdb[module_pre]][dirname]}&file=form_content&job=view&mid=$fidDB[id]&id=$id</a>",0);
...
function get_out_pic($str,$getpic=1){
	...
	foreach( $listdb AS $key=>$value){
		$filedb[]=$value;
		$name=rands(5)."__".preg_replace("/(.*)\.(jpg|png|gif)$/is",".\\2",$value);
