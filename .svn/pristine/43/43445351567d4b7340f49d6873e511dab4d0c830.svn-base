...
function tempdir($file,$type=''){
	...
	if($type=='pwbbs'){
		...
		return $webdb[mirror].(eregi('\/$',$webdb[mirror])?'':'/').$file;
	}else{
...
function getpage($table,$choose,$url,$rows=20,$total=''){
	...
	$showpage="<a href=\"$url&page=1\" title=\"首页\">首页</A> <a href=\"$url&page=$uppage\" title=\"上一页\">上一页</A>  {$show}  <a href=\"$url&page=$nextpage\" title=\"下一页\">下一页</A> <a href=\"$url&page=$totalpage\" title=\"尾页\">尾页</A> <a href='' ><font color=red>当前页：$page</font>  总页：$totalpage 共计 ：$total 条 </a>";
    if($totalpage>1){
		return $showpage;
	}
}
