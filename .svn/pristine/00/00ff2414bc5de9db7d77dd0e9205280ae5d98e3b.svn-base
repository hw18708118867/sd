...
isset($_POST['listdb']) && $listdb = $_POST['listdb'];
...
elseif($job=="show_iframe"){
	...
	if($type=="list_atc"||$type=='all')
	{
		...
		foreach($aidsdb AS $key=>$value){
			...
			if($_listdb[$value]){
				$show.="<tr> 
                <td>{$_listdb[$value][aid]}</td>
                <td class='tL'>{$_listdb[$value][subject]}</td>
				<td><input type='text' name='listdb[{$value}]' size='5' value='{$NUM}0'></td>
                <td><A HREF='special.php?job=show_iframe&id=$id&type=list_atc&act=del&aid={$_listdb[$value][aid]}' target='spiframe'>移除</A></td>
              </tr>";
			}
		}
		$show="<table width='100%' border='0' cellspacing='0' cellpadding='0' class='ListTable'>
              <tr> 
                <th width='5%'>ID</th>
                <th width='74%'>标 题</th>
				<th width='10%'>排序值</th>
	...
	if($type=='myatc'||$type=='all')
	{
		...
		while($rs = $db->fetch_array($query)){
			...
			$show.="<tr> 
                <td>{$rs[aid]}</td>
                <td class='tL'><A HREF='..$webdb[path]/bencandy.php?fid=$rs[fid]&id=$rs[aid]' target=_blank>{$rs[title]}</A></td>
                <td>&nbsp;$add</td>
              </tr>";
		}
		$show="<table width='100%' border='0' cellspacing='0' cellpadding='0' class='ListTable'>
...
elseif($job=="show_BBSiframe"){
	...
	if($type=="list_atc"||$type=='all')
	{
		...
		foreach($aidsdb AS $key=>$value){
			...
			if($_listdb[$value]){
				$show.="<tr> 
                <td>{$_listdb[$value][tid]}</td>
                <td class='tL'>{$_listdb[$value][subject]}</td>
				<td><input type='text' name='listdb[{$value}]' size='5' value='{$NUM}0'></td>
                <td><A HREF='special.php?job=show_BBSiframe&id=$id&type=list_atc&act=del&aid={$_listdb[$value][tid]}' target='spiframe'>移除</A></td>
              </tr>";
			}
		}
		$show="<table width='100%' border='0' cellspacing='0' cellpadding='0' class='ListTable'>
	...
	if($type=='myatc'||$type=='all')
	{
		...
		if(ereg("^pwbbs",$webdb[passport_type])){			
			while($rs = $db->fetch_array($query)){
				...
				$show.="<tr> 
					<td>{$rs[tid]}</td>
					<td class='tL'><a href='$webdb[passport_url]/read.php?tid=$rs[tid]' target=_blank>$rs[subject]</a></td>
					<td>&nbsp;$add</td>
				  </tr>";
			}
			...
			while($rs = $db->fetch_array($query)){
				...
				$show.="<tr> 
					<td>{$rs[tid]}</td>
					<td class='tL'><a href='$webdb[passport_url]/viewthread.php?tid=$rs[tid]' target=_blank>$rs[subject]</a></td>
					<td>&nbsp;$add</td>
				  </tr>";
			}
		}
